...
FROM 'file:///01-01-2021.csv' AS line

WITH coalesce(line.`Province_State`, 'General') as province,
     coalesce(line.`Country_Region`, 'Missing') as region,
     datetime(line.`Last_Update`) as lastUpdate,
     date(datetime({ epochMillis: apoc.date.parse(replace("01-01-2021", ".csv", ""), 'ms', 'MM-dd-yyyy') })) as reportDate,
